...
app.get('/wells/:id', function (req, res){
    ...
    var queryString = 'SELECT w1.site_number, w1.stainset_number,  w1.w1_filepath, p1.well_name, p1.UCSC_CSC_plate, p1.Cell_lines, p1.Magnification, p1.TimePoint, p1.experiment_date FROM wavelength_1 w1 INNER JOIN plate_wells p1 ON(p1.UCSC_CSC_plate = w1.plate_name AND p1.well_name = w1.well_name AND p1.Cell_lines = w1.cell_line AND p1.timepoint = w1.timepoint AND p1.Magnification = w1.Magnification) WHERE p1.id= 31492 UNION SELECT w2.site_number, w2.stainset_number, w2.w2_filepath, p1.well_name, p1.UCSC_CSC_plate, p1.Cell_lines, p1.Magnification, p1.TimePoint, p1.experiment_date FROM wavelength_2 w2 INNER JOIN plate_wells p1 ON(p1.UCSC_CSC_plate = w2.plate_name AND p1.well_name = w2.well_name AND p1.Cell_lines = w2.cell_line AND p1.timepoint = w2.timepoint AND p1.Magnification = w2.Magnification)WHERE p1.id=' + well_id;
